 ...
 */
import type { BigNumberish, BytesLike, Numeric } from 'ethers-v6'
import {
  assert,
  assertArgument,
  assertPrivate,
...
// import { fromTwos, getBigInt, getNumber, mask, toBigInt } from './maths.js'
...
*/
  ...
  #add(o: FixedNumber, safeOp?: string): FixedNumber {
    this.#checkFormat(o)
    return this.#checkValue(this.#val + o.value, safeOp)
  }
  ...
  #mul(o: FixedNumber, safeOp?: string): FixedNumber {
    this.#checkFormat(o)
    return this.#checkValue((this.#val * o.value) / this.#tens, safeOp)
  }
  ...
  mulSignal(other: FixedNumber): FixedNumber {
    ...
    const value = this.#val * other.value
  ...
  #div(o: FixedNumber, safeOp?: string): FixedNumber {
    assert(o.value !== BN_0, 'division by zero', 'NUMERIC_FAULT', {
      operation: 'div',
      fault: 'divide-by-zero',
      value: this
    })
    this.#checkFormat(o)
    return this.#checkValue((this.#val * this.#tens) / o.value, safeOp)
  }
  ...
  divSignal(other: FixedNumber): FixedNumber {
    assert(other.value !== BN_0, 'division by zero', 'NUMERIC_FAULT', {
      operation: 'div',
      fault: 'divide-by-zero',
      value: this
    })
    ...
    assert(value % other.value === BN_0, 'precision lost during signalling div', 'NUMERIC_FAULT', {
      operation: 'divSignal',
      fault: 'underflow',
      value: this
    })
    return this.#checkValue(value / other.value, 'divSignal')
  }
  ...
  static fromString30(_value: string): FixedNumber {
    return FixedNumber.fromString(_value, 30)
  }
...
export function tenPow(decimals: number): FixedNumber {
  if (decimals >= 0) return FixedNumber.fromString('1' + '0'.repeat(decimals))
  else return FixedNumber.fromString('0.' + '0'.repeat(-decimals - 1) + '1')
}

export function maxFN(f1: FixedNumber, f2: FixedNumber): FixedNumber {
  return f1.gte(f2) ? f1 : f2
}
