...
import { divFN, mulFN } from '../fixedNumber'
import type { Token } from '../src/common/tokens'
import type { IAdapterV1 } from '../src/interfaces/V1/IAdapterV1'
import type {
  AmountInfo,
  ApiOpts,
  CreateOrder,
...
import type { MarketTag, MarketWithMetadata, MarketWithPreview, RouteData, TokenWithPrice } from './Route'
import {
...
export class AutoRouterV1 extends ConsolidatedRouterV1 {
  ...
  private _getCreateOrder(
    ...
    const sizeDeltaAmountInfo: AmountInfo = amountInfoInToken.sizeDeltaInToken
    ...
    const marginDeltaAmountInfo: AmountInfo = amountInfoInToken.collateralDeltaInToken
  ...
  async _getTradePreview(
    ...
    const outPromise = (await Promise.allSettled(promises)).filter((v) => v.status == 'fulfilled')
    const out = outPromise.map((o) => (o as any).value)
  ...
  async getBestRoute(routeData: RouteData, opts?: ApiOpts): Promise<OpenTradePreviewInfo> {
    ...
    const bestRoute = routes.reduce(reduceCallback)
