...
export const ORDERLY_ERRORS = ['orderly credentials missing', 'headers not set'].map((e) => e.toLowerCase())
  ...
  try {
    ...
    switch (true) {
      ...
      case 'message' in e && DYDX_ERRORS.includes(e.message.toLowerCase()):
        ...
        break
      case 'message' in e && ORDERLY_ERRORS.includes(e.message.toLowerCase()):
        // console.error(`Orderly credentials missing: Doing Early return`)
        break
