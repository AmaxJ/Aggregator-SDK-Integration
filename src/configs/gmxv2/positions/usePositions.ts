...
import type { ContractMarketPrices, MarketsData } from '../markets/types'
  ...
  const { data: positionsData } = await useMulticall(chainId, 'usePositionsData', {
    ...
    parseResponse: (res) => {
      ...
      return positions.reduce((positionsMap: PositionsData, positionInfo: any, i: number) => {
        ...
        const { account: account_, market: marketAddress, collateralToken: collateralTokenAddress } = addresses
        ...
        const positionKey = getPositionKey(account_, marketAddress, collateralTokenAddress, flags.isLong)
