...
import type { MarketsInfoData } from './types'
...
export async function useMarketsInfoOld(chainId: number, wallet: string, opts?: ApiOpts): Promise<MarketsInfoResult> {
  const account = wallet

  const { marketsData, marketsAddresses, tokensData, pricesUpdatedAt } = await _getMarketsAndTokensData(
    chainId,
    wallet,
    opts
  )
        ...
        parseResponse: (res: any) => {
          return marketsAddresses!.reduce((acc: MarketsInfoData, marketAddress) => {
            ...
            if (!readerValues || !dataStoreValues || readerErrors || dataStoreErrors) {
              // eslint-disable-next-line no-console
              console.error('market info error', marketAddress, readerErrors, dataStoreErrors, dataStoreValues)
              return acc
            }
...
async function _getMarketsAndTokensData(chainId: number, wallet: string, opts?: ApiOpts) {
  // internally caching
  ...
  return { marketsData, marketsAddresses, tokensData, pricesUpdatedAt }
}
...
export async function useMarketsInfo(chainId: number, wallet: string, opts?: ApiOpts): Promise<MarketsInfoResult> {
  ...
  if (!marketsAddresses || !tokensData) return {}

  const sTime = getStaleTime(CACHE_SECOND * 30, opts)

  const marketsInfoData: MarketsInfoData = {}

  for (let i = 0; i < marketsAddresses.length; i++) {
    const marketAddr = marketsAddresses[i]

    const { data } = await cacheFetch({
      key: [GMXV2_CACHE_PREFIX, 'useMulticall', 'useMarketsInfo', marketAddr, chainId],
      fn: () =>
        useMulticall(chainId, 'useMarketsInfo', {
          key: marketsAddresses.length > 0 && [marketsAddresses.join('-'), dataStoreAddress, account, pricesUpdatedAt],
          ...
          request: () =>
            marketsAddresses!
              .filter((m) => m.toLowerCase() === marketAddr.toLowerCase())
              .reduce((request, marketAddress) => {
                const market = getByKey(marketsData, marketAddress)!
                const marketPrices = getContractMarketPrices(tokensData!, market)!
          ...
          parseResponse: (res: any) => {
            return marketsAddresses!
              .filter((m) => m.toLowerCase() === marketAddr.toLowerCase())
              .reduce((acc: MarketsInfoData, marketAddress) => {
                const readerErrors = res.errors[`${marketAddress}-reader`]
                const dataStoreErrors = res.errors[`${marketAddress}-dataStore`]
    ...
    Object.assign(marketsInfoData, data)
  }
