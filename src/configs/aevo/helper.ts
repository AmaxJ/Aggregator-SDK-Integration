import { abs, bipsDiff, FixedNumber } from '../../../fixedNumber'
import { calcActualPrecision } from '../../common/helper'
import type { OBData, OBLevel } from '../../interfaces/V1/IRouterAdapterBaseV1'
import { LEV_OUT_OF_BOUNDS } from '../hyperliquid/hlErrors'
import type { AevoWssOrderBook } from './aevoWsClient'
...
export function toNearestTickBN(input: bigint, tickSize: bigint): string {
  if (tickSize <= 0n) {
    throw new Error('Tick size must be greater than zero')
  }

  return ((input * tickSize) / tickSize).toString()
}
...
export function aevoMapAevoObToObData(ob: AevoWssOrderBook): OBData {
  ...
  const obData: OBData = {
    actualPrecision: calcActualPrecision(bids, asks),
    bids: bids,
    asks: asks,
    spread: spread,
