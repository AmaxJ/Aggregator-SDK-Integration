...
import type { AevoAdapterV1 } from '../../exchanges/aevo'
import { AEVO_REF_CODE } from '../../exchanges/aevo'
import type { APICallParamsWithMetadata, RequestSignerFnWithMetadata } from '../../interfaces/IActionExecutor'
  ...
  return {
    fn: async (wallet: WalletClient) => {
      ...
      const signableOrder = {
        ...
        limitPrice:
          updatedOrder.limit_price === maxUint256.toString()
            ? BigInt(updatedOrder.limit_price)
            : BigInt(Math.round(Number(updatedOrder.limit_price))),
  ...
  return {
    fn: async (wallet: WalletClient) => {
      ...
      const signableOrder = {
        ...
        isBuy: order.is_buy,
        limitPrice:
          order.limit_price === maxUint256.toString()
            ? BigInt(order.limit_price)
            : BigInt(Math.round(Number(order.limit_price))),
