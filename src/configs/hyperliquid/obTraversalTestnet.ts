import { addFN, FixedNumber, mulFN, subFN } from '../../../fixedNumber'
import { CACHE_SECOND, CACHE_TIME_MULT, cacheFetch, getStaleTime, HL_CACHE_PREFIX } from '../../common/cache'
import type { TraverseResult } from '../../common/types'
import type { ApiOpts, TradeDirection } from '../../interfaces/V1/IRouterAdapterBaseV1'
import { getL2Book } from './api/clientTestnet'
import { HL_TAKER_FEE_BPS } from './api/configTestnet'
import type { L2Book } from './api/types'
import { hlGetCachedL2Book } from './api/wsclientTestnet'
...
export async function traverseHLBook(
  ...
  const denominator = size.subFN(remainingSize)
  const avgExecPrice = denominator.isZero() ? FixedNumber.fromValue(0) : avgPriceAcc.divFN(denominator)
