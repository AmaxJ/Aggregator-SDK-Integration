...
import { IERC20__factory, VaultReader__factory } from '../../../typechain/gmx-v1'
import {
  CACHE_MINUTE,
  CACHE_SECOND,
  CACHE_TIME_MULT,
...
import { getBNFromFN, toAmountInfo, toNumberDecimal } from '../../common/helper'
import type { ExtendedMarket, ExtendedPosition, Order, ViewError } from '../../interfaces/interface'
...
function getNextData(
  ...
  if (existingPosition) {
    ...
    const tempNextAveragePrice = getNextAveragePrice(
      existingPosition.size,
      toUsdMax,
      nextHasProfit,
      nextDelta,
...
export const getEditCollateralPreviewInternal = async (
  ...
  const nextCollateral = isDeposit
  ...
  const executionFees = getUsd(execFee, getContract(ARBITRUM, 'NATIVE_TOKEN')!, false, infoTokens)
...
export const getEditCollateralPreviewInternalV1 = async (
  ...
  const collateralToken = existingPos.collateral
  const provider = rpc[ARBITRUM]
  const position = existingPos.metadata
  const isLong = position.direction ? position.direction == 'LONG' : position.isLong

  const fromAmount = getBNFromFN(marginDelta.amount)
  // fromAmount = isDeposit ? fromAmount : fromAmount.mul(expandDecimals(1, 30 - Number(collateralToken.decimals)))
  let fromAmountUsd: BigNumber
  ...
  const executionFees = getUsd(executionFee, getContract(ARBITRUM, 'NATIVE_TOKEN')!, false, infoTokens)
  depositFeeUSD = depositFeeUSD.add(executionFees || BigNumber.from(0))
...
export const getCloseTradePreviewInternal = async (
  ...
  const isClosing = closeSize.eq(position.size)
  const fromAmount = closeSize
  const sizeDelta = fromAmount
  let receiveUsd = BigNumber.from(0)
  ...
  const collateralDelta = BigNumber.from(0)
  let totalFees = BigNumber.from(0)
  let positionFee = getMarginFee(fromAmount)
  const fundingFee = position.borrowFee!
  let nextLeverage
  ...
  const { nextDelta, nextHasProfit } = getNextDataForClose(position, !isTrigger, triggerPrice)

  const leverageWithoutDelta = getLeverage({
    size: position.size,
    collateral: position.collateral,
  ...
  const collateralTokenInfo = getTokenInfo(infoTokens, position.originalCollateralToken!)
  const receiveTokenInfo = outputToken ? getTokenInfo(infoTokens, outputToken.address) : undefined

  receiveUsd = applySpread(receiveUsd, collateralTokenInfo?.spread)

  const convertedAmount = fromAmount
  ...
  const isError = isNotEnoughReceiveTokenLiquidity || isCollateralPoolCapacityExceeded
...
export const getCloseTradePreviewInternalV1 = async (
  ...
  const closeSize = getBNFromFN(closePositionData.closeSize.amount)
  const position = positionInfo.metadata
  const provider = rpc[ARBITRUM]
  ...
  let totalFees = BigNumber.from(0)
  let positionFee = getMarginFee(fromAmount)
  const fundingFee = position.borrowFee ? position.borrowFee : position.fundingFee
  let nextLeverage
  const usdgToken = IERC20__factory.connect(USDG_ADDRESS, provider)
  const isTrigger = closePositionData.type !== 'MARKET'
  const triggerPrice = isTrigger ? getBNFromFN(closePositionData.triggerData!.triggerPrice) : undefined
  ...
  const receiveTokenInfo = outputToken ? getTokenInfo(infoTokens, outputToken.address[ARBITRUM]!) : undefined
...
export const checkTradePathLiquidiytInternal = async (
  ...
  const collateralTokenInfo = getTokenInfo(infoTokens, order.inputCollateral.address)
  const toTokenInfo = getTokenInfo(infoTokens, market.marketToken!.address)
...
export const getTradePreviewInternal = async (
  ...
  const { nextAveragePrice, nextDelta, nextHasProfit } = getNextData(
  ...
  const marketCollateralToken = order.direction == 'LONG' ? market.marketToken!.address! : USDC_E_ADDRESS
  ...
  const positionFee = toUsdMax!.mul(MARGIN_FEE_BASIS_POINTS).div(BASIS_POINTS_DIVISOR)
  ...
  const isError = isNotEnoughReceiveTokenLiquidity || isCollateralPoolCapacityExceeded || isNotEnoughAvailableLiq
...
export const getTradePreviewInternalV1 = async (
  ...
  const marketCollateralToken = orderData.direction == 'LONG' ? market.indexToken.address[ARBITRUM]! : USDC_E_ADDRESS
  ...
  const collateralTokenInfo = getTokenInfo(infoTokens, orderData.collateral.address[ARBITRUM]!)
  const toTokenInfo = getTokenInfo(infoTokens, market.indexToken.address[ARBITRUM]!)
