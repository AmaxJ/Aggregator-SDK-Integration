...
import { HL_MAKER_FEE_BPS, HL_TAKER_FEE_BPS, hyperliquid } from '../configs/hyperliquid/api/config'
import type {
  ActiveAssetData,
  AllMids,
  AssetCtx,
...
export class HyperliquidAdapterV1 implements IAdapterV1 {
  ...
  async setCredentials(auth: AuthParams<this['protocolId']>): Promise<void> {
    throw new Error('Method not implemented.')
  }
      ...
      meta.universe.forEach((u) => {
        if (u.name !== 'STRAX') {
          ...
          const staticMetadata: GenericStaticMarketMetadata = {
            ...
            minLimitPositionSize: FixedNumber.fromValue(this.minPositionUsd.toString(), 30, 30),
            maxPrecision: 4,
  ...
  async increasePosition(orderData: CreateOrder[], wallet: string, opts?: ApiOpts | undefined): Promise<ActionParam[]> {
    ...
    for (const each of orderData) {
      ...
      const availableToTrade = await this.getAvailableToTrade(
        wallet,
        undefined as AvailableToTradeParams<'HL' & this['protocolId']>,
        opts
      )
      ...
      const orderData_: OrderRequest['order_type'] = { limit: { tif: toTif(each.tif || 'GTC') } }
  ...
  async updateOrder(orderData: UpdateOrder[], wallet: string, opts?: ApiOpts | undefined): Promise<ActionParam[]> {
    ...
    for (const each of orderData) {
      ...
      if (order.reduceOnly === true && !each.triggerData.triggerLimitPrice) {
        triggerData.triggerLimitPrice = FixedNumber.fromString(
          slippagePrice(isBuy, slippage, Number(triggerData.triggerPrice._value)).toFixed(18)
        )
      }

      // populate trigger data if required
      let orderData_: ModifyRequest['order']['order_type'] = { limit: { tif: 'Gtc' } }
      ...
      if (triggerData.triggerLimitPrice) {
        ...
        ;({ orderData: orderData_, limitPrice } = populateTrigger(isBuy, price, each.orderType, triggerData))
      } else {
        // covers following handling:
        // - basic limit order which executes at specified price
        orderData_ = { limit: { tif: 'Gtc' } }
        limitPrice = roundedPrice(Number(triggerData.triggerPrice._value))
      }
  ...
  async closePosition(
    ...
    for (let i = 0; i < positionInfo.length; ++i) {
      ...
      const slippage = closeData.slippage ? closeData.slippage / 100 : 0.01
  ...
  async getAllPositions(
    ...
    assetPositions.forEach((ap, index) => {
      ...
      const size = FixedNumber.fromString(position.szi)
  ...
  _isSpotFill(fill: UserFill): boolean {
    return fill.dir === 'Buy' || fill.dir === 'Sell'
  }
  ...
  async getTradesHistory(
    ...
    const userFills = (await getUserFills(wallet)).filter((uf) => !this._isSpotFill(uf) && uf.liquidationMarkPx == null)
  ...
  async getOpenTradePreview(
    ...
    const sTimeW2D = getStaleTime(CACHE_SECOND * 2, opts)
