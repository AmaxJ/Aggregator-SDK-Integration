...
import { addFN, FixedNumber, mulFN } from '../../fixedNumber'
import {
  AevoClient,
...
import type { L1SocketDepositHelper } from '../../typechain/aevo'
import { L1SocketDepositHelper__factory } from '../../typechain/aevo'
import type { IERC20 } from '../../typechain/gmx-v1'
import { IERC20__factory } from '../../typechain/gmx-v1'
import {
  AEVO_DEPOSIT_H,
  CANCEL_ORDER_H,
  EMPTY_DESC,
...
import type { FarmBoostReturnType } from '../configs/aevo/aevoCacheHelper'
import {
  aevoCacheGetAccount,
  aevoCacheGetAllMarkets,
  aevoCacheGetCoingeckoStats,
  aevoCacheGetFarmBoost,
...
import { getAevoWssOrderBook, getAevoWssTicker, openAevoWssConnection } from '../configs/aevo/aevoWsClient'
import {
  aevo,
...
export class ExtendedAevoClient extends AevoClient {
  ...
  public clearCredentials() {
    delete this.headers['AEVO-KEY']
    delete this.headers['AEVO-SECRET']
...
export class AevoAdapterV1 implements IAdapterV1 {
  ...
  private contracts: Record<SupportedChains, L1SocketDepositHelper | undefined> = {
  ...
  clearCredentials(): void {
    this.aevoClient.clearCredentials()
  }

  async setCredentials(auth: AuthParams<this['protocolId'] & 'AEVO'>): Promise<void> {
  ...
  async deposit(params: DepositWithdrawParams[]): Promise<ActionParam[]> {
    ...
    for (const each of params) {
      ...
      const address = each.token.address[each.chainId as keyof Token['address']]
  ...
  async getAgentState(wallet: string, agentParams: AgentParams[], opts?: ApiOpts): Promise<AgentState[]> {
    ...
    const v = await this.privateApi.getAccount()

    const isAuthenticated =
      (v.signing_keys?.some((k) => k.signing_key === agent.agentAddress) || false) && v.account === wallet
  ...
  async withdraw(params: DepositWithdrawParams[]): Promise<ActionParam[]> {
    ...
    for (const each of params) {
      ...
      const msgGasLimit = withdrawGasLimits[each.chainId as keyof Token['address']]
    ...
    if (chains == undefined || chains.includes(aevo)) {
      ...
      allMarkets.forEach((m) => {
        ...
        const staticMetadata: GenericStaticMarketMetadata = {
          ...
          minLimitPositionSize: FixedNumber.fromString(m.min_order_value),
          maxPrecision: 1,
  ...
  async getMarketPrices(marketIds: string[], opts?: ApiOpts | undefined): Promise<FixedNumber[]> {
    ...
    if (!cachedTickers.includes(undefined)) {
      // return from cache
      return cachedTickers.map((t) => {
        return FixedNumber.fromString(t!.index_price).isZero()
          ? FixedNumber.fromString(t!.mark.price)
          : FixedNumber.fromString(t!.index_price)
      })
    } else {
      ...
      return marketIds.map((marketId) => {
        ...
        return market
          ? FixedNumber.fromString(market.index_price).isZero()
            ? FixedNumber.fromString(market.mark_price)
            : FixedNumber.fromString(market.index_price)
          : ZERO_FN
      })
    }
  }
  ...
  async getDynamicMarketMetadata(marketIds: string[], opts?: ApiOpts | undefined): Promise<DynamicMarketMetadata[]> {
    ...
    if (!cachedTickers.includes(undefined)) {
      ...
      cachedTickers.forEach((t, index) => {
        ...
        const iPrice = FixedNumber.fromString(t!.index_price).isZero()
          ? FixedNumber.fromString(t!.mark.price)
          : FixedNumber.fromString(t!.index_price)
      ...
      for (let i = 0; i < marketIds.length; i++) {
        ...
        if (cgStat) {
          ...
          let iPrice = cgStat.index_price ? FixedNumber.fromString(cgStat.index_price) : ZERO_FN
          iPrice = iPrice.isZero() ? (await this.getMarketPrices([mId]))[0] : iPrice
          const oiUsd = oiToken.mulFN(iPrice).divFN(FixedNumber.fromString('2'))
  ...
  async increasePosition(orderData: CreateOrder[], wallet: string, opts?: ApiOpts | undefined): Promise<ActionParam[]> {
    ...
    // to ensure AEVO_TOKENS_MAP is present
    const sTimeMarkets = getStaleTime(CACHE_DAY, opts)
    await aevoCacheGetAllMarkets(this.publicApi, sTimeMarkets, sTimeMarkets * CACHE_TIME_MULT, opts)
  ...
  async updateOrder(orderData: UpdateOrder[], wallet: string, opts?: ApiOpts | undefined): Promise<ActionParam[]> {
    ...
    for (const each of orderData) {
      ...
      const tif = each.tif ? (each.tif === 'GTC' ? time_in_force.GTC : time_in_force.IOC) : time_in_force.GTC

      let tpsl = undefined
      let limitPriceAdjusted = undefined
      let triggerPriceAsjusted = undefined

      const triggerData = { ...each.triggerData }

      if (triggerData.triggerLimitPrice) {
        // covers following handling:
        // - stop limit and stop market (and therefore TP / SL market & limit)
        const { orderData: orderData_, limitPrice } = populateTrigger(isBuy, price, each.orderType, triggerData)

        if (!orderData_ || !orderData_.trigger) throw new Error('trigger expected but not found')

        limitPriceAdjusted = to6Decimals(toNearestTick(limitPrice, Number(marketInfo.priceStep)))
        triggerPriceAsjusted = to6Decimals(toNearestTick(orderData_.trigger.triggerPx, Number(marketInfo.priceStep)))
      } else {
        // add slippage to stop loss or take profit
        if (order.reduce_only === true || (order.trigger && order.stop)) {
          triggerPriceAsjusted = to6Decimals(
            toNearestTick(Number(triggerData.triggerPrice._value), Number(marketInfo.priceStep))
          )
          limitPriceAdjusted = isBuy ? maxUint256.toString() : '0'
          tpsl =
            each.orderType === 'STOP_LOSS' || each.orderType === 'STOP_LOSS_LIMIT'
              ? 'sl'
              : each.orderType === 'TAKE_PROFIT' || each.orderType === 'TAKE_PROFIT_LIMIT'
                ? 'tp'
                : undefined
        } else {
          // covers following handling:
          ...
          limitPriceAdjusted = to6Decimals(
            toNearestTick(
              slippagePrice(isBuy, 0.01, Number(triggerData.triggerPrice._value)),
          ...
          //   toNearestTick(Number(triggerData.triggerPrice._value), Number(marketInfo.priceStep))
          // )
          triggerPriceAsjusted = undefined
        }
      }
      ...
      const request: NonNullable<Parameters<AevoAdapterV1['privateApi']['postOrdersOrderId']>[1]> = {
        ...
        trigger: triggerPriceAsjusted ? triggerPriceAsjusted.toString() : undefined
      }

      if (order.reduce_only) {
        request.reduce_only = order.reduce_only
      }
  ...
  async cancelOrder(orderData: CancelOrder[], _: string, opts?: ApiOpts | undefined): Promise<ActionParam[]> {
    ...
    for (const each of orderData) {
      ...
      const fn: RequestSignerFn = async (__: WalletClient) =>
    ...
    for (let i = 0; i < positionInfo.length; ++i) {
      ...
      if (closeData.type == 'MARKET') {
        ...
        const request: NonNullable<Parameters<AevoAdapterV1['privateApi']['postOrders']>[0]> = {
          ...
          is_buy: isBuy,
          amount: to6Decimals(sizeDelta).toString(),
          // TODO: viem has bug, after updating, make it just MaxUint256
          limit_price: isBuy ? maxUint256.toString() : '0',
          salt: '', // will be filled internally
          signature: '', // will be filled internally
          timestamp: '', // will be filled internally
          // post_only: false,
      ...
      //     isBuy ? '115792089237316200000000000000000000000000000000000000000000000000000000' : '0'
      //   )
      // }
      ...
      const { orderData } = populateTrigger(isBuy, price, closeData.type, closeData.triggerData, true)

      const triggerLimitPrice = isBuy ? maxUint256 : 0n
    ...
    for (let i = 0; i < ordersData.length; i++) {
      ...
      if (od.stop) {
        ...
        if (od.reduce_only) {
          // can be TP / SL
          orderType = od.stop
        } else if (BigInt(od.price) > 2n ** 128n || od.price === '0') {
          orderType = od.stop
        } else {
          // can be TPL / SLL
      ...
      let triggerLimitPrice: FixedNumber | undefined = undefined
      switch (orderType) {
        case 'LIMIT':
  ...
  async getCloseTradePreview(
    ...
    for (let i = 0; i < positionInfo.length; i++) {
      ...
      if (isMarket) {
        const cachedOB = getAevoWssOrderBook(asset)
        const sTimeOB = getStaleTime(CACHE_SECOND * 5, opts)
        const ob = cachedOB ? cachedOB : await aevoCacheGetOrderbook(asset, this.publicApi, sTimeOB, sTimeOB, opts)
        ...
        if (!closeSize.isZero() && !traResult.remainingSize.isZero()) {
          isError = true
          errMsg = NOT_ENOUGH_LIQUIDITY
          previewsInfo.push(closePreErrRes(pos.marketId, true, true, AEVO_COLLATERAL_TOKEN, errMsg))
          continue
        }
      }
  ...
  async getAccountInfo(wallet: string, opts?: ApiOpts | undefined): Promise<AccountInfo[]> {
    ...
    const mmUtilizationPercent = equity.isZero()
      ? ZERO_FN
      : maintainenceMarginUsed.divFN(equity).mulFN(FixedNumber.fromString('100'))
    const imUtilizationPercent = equity.isZero()
      ? ZERO_FN
      : initialMarginUsed.addFN(maintainenceMarginUsed).divFN(equity).mulFN(FixedNumber.fromString('100'))
    ...
    const collaterals = accountInfo.collaterals || []
    for (const asset of Object.keys(pendingWithdrawsMap)) {
      const pws = pendingWithdrawsMap[asset]
      const collateral = collaterals.find((c) => c.collateral_asset == asset)
      if (collateral) {
  ...
  async getFarmBoostData(wallet: string, opts?: ApiOpts | undefined): Promise<FarmBoostReturnType> {
    this._setCredentials(opts, true)

    return aevoCacheGetFarmBoost(this.privateApi, 0, 0, opts)
  }
