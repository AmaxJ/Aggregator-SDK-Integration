...
import { applySlippage, getBNFromFN, getPaginatedResponse, toAmountInfo, validDenomination } from '../common/helper'
...
import type { GMX_V2_TOKEN } from '../configs/gmxv2/gmxv2Tokens'
import { getGmxV2TokenByAddress, GMX_V2_COLLATERAL_TOKENS, GMX_V2_TOKENS } from '../configs/gmxv2/gmxv2Tokens'
...
import type { PositionOrderInfo } from '../configs/gmxv2/orders'
import { isMarketOrderType, isOrderForPosition } from '../configs/gmxv2/orders'
import type { OrdersInfoData } from '../configs/gmxv2/orders/types'
import { OrderType as InternalOrderType } from '../configs/gmxv2/orders/types'
import { useOrdersInfo } from '../configs/gmxv2/orders/useOrdersInfo'
import type { PositionInfo as InternalPositionInfo } from '../configs/gmxv2/positions/types'
...
import type { TokensData } from '../configs/gmxv2/tokens/types'
import { useTokensData } from '../configs/gmxv2/tokens/useTokensData'
import { convertToTokenAmount, convertToUsd, getIsEquivalentTokens, getTokenData } from '../configs/gmxv2/tokens/utils'
...
export class GmxV2Service implements IAdapterV1 {
  ...
  > {
    ...
    const res = cacheFetch({
      ...
      fn: async () => {
        const marketPropsPromise = this.reader.getMarkets(this.DATASTORE_ADDR, 0, 1000)
        const marketsInfoDataPromise = useMarketsInfo(ARBITRUM, ethers.constants.AddressZero, opts)

        const [marketProps, { marketsInfoData }] = await Promise.all([marketPropsPromise, marketsInfoDataPromise])
        ...
        for (const mProp of marketProps) {
          ...
          const minCollateralFactor = marketsInfoData![mProp.marketToken].minCollateralFactor
          const maxLeverageBasisMCF = getMaxLeverageByMinCollateralFactor(minCollateralFactor.toBigInt()) / 2

          const staticMetadata: GenericStaticMarketMetadata = {
            maxLeverage: FixedNumber.fromValue(maxLeverageBasisMCF, 4, 4),
            ...
            minLimitPositionSize: FixedNumber.fromValue('0', 30, 30),
            maxPrecision: 1,
  ...
  async increasePosition(orderData: CreateOrder[], wallet: string, opts?: ApiOpts): Promise<ActionParam[]> {
    ...
    for (const od of orderData) {
      ...
      // prepare calldata
      const orderTx = await this.exchangeRouter.populateTransaction.createOrder({
        addresses: {
          receiver: wallet,
  ...
  async closePosition(
    ...
    for (let i = 0; i < positionInfo.length; i++) {
      ...
      acceptablePrice = applySlippage(
        acceptablePrice,
        closePositionData[i].slippage ?? DEFAULT_ACCEPTABLE_PRICE_SLIPPAGE,
  ...
  async updatePositionMargin(
    ...
    for (let i = 0; i < positionInfo.length; i++) {
      ...
      if (updatePositionMarginData[i].collateral.symbol === 'ETH' && updatePositionMarginData[i].isDeposit) {
        orderTx.value = BigNumber.from(updatePositionMarginData[i].margin.amount.value).add(orderTx.value || 0)
        requiresErc20Token = false
      }
  ...
  async claimFunding(wallet: string, opts?: ApiOpts): Promise<ActionParam[]> {
    ...
    const { marketsInfoData } = await useMarketsInfo(ARBITRUM, wallet)
  ...
  async _getOrderInfo(wallet: string): Promise<OrdersInfoData> {
    ...
    const { ordersInfoData } = await useOrdersInfo(ARBITRUM, {
      account: wallet,
      marketsInfoData,
      positionsInfoData,
      tokensData
  ...
  private _getPriceForRawTrade(priceMap: number[], fromTS: number, rawTradeTimestamp: any, feeInEth: string) {
    ...
    const PRECISION_ = 10n ** 30n

    return (
      (BigInt(this._checkNull(feeInEth)) * PRECISION_ * etherPrice) /
  ...
  private async _getPriceMap(rawTrades: any[]) {
    const from = new Date(rawTrades[rawTrades.length - 1].executedTxn.timestamp * 1000)
    from.setUTCHours(0, 0, 0, 0)
    const fromTS = from.getTime() / 1000

    const to = new Date(rawTrades[0].executedTxn.timestamp * 1000)
    ...
    try {
      ...
      const priceMap = new Array<number>()
  ...
  async getOpenTradePreview(
    ...
    const { marketsInfoData, tokensData } = await useMarketsInfo(ARBITRUM, wallet, opts)
  ...
  async getUpdateMarginPreview(
    ...
    const { tokensData } = await useTokensData(ARBITRUM, wallet, opts)
    ...
    for (let i = 0; i < existingPos.length; i++) {
      ...
      const { nextCollateralUsd, nextLeverage, nextLiqPrice, totalFeesUsd } = await getNextUpdateMarginValues(
        isDeposit[i],
  ...
  async getTotalClaimableFunding(wallet: string, opts?: ApiOpts): Promise<FixedNumber> {
    const { marketsInfoData } = await useMarketsInfo(ARBITRUM, wallet, opts)
    const markets = Object.values(marketsInfoData ?? {})
